...
|---|---|---|
| `search_knowledge` | `(token, query, top_k=None → config TOP_K, include_archived=False)` | Embed query → dual-scope ACL-filtered search (below). Returns chunks with full citation metadata (`doc_id, title, page, period, source, status, superseded_by, score, text`). Empty → structured `{status: "no_result"}`, distinct from errors. |
...
2. **Chunk** (`chunker.py`) — **heading-aware section splitting**, never crossing page boundaries (citations stay page-exact), hard cap 125 tokens/chunk (tiktoken `cl100k_base`), with a small sibling-chunk tail overlap (10 tokens, config). This corpus extracts with no blank lines, so headings are the structure signal: a short line (<60 chars) without terminal punctuation starts a new section; over-detection (e.g. table rows) is harmless because consecutive sections are re-packed greedily up to the cap. A confidential-marker line **always** starts its own section; marker sections are kept whole and alone, and **overlap never crosses a confidential boundary** — a restricted section can never share a chunk (and thus an ACL) with general text. Oversized sections fall back to line-level splitting. Deterministic `chunk_id = {doc_id}:{page}:{seq}`.
   - Trade-offs: the heading heuristic is tuned to this corpus's conventions; a marker section runs to the next heading or page end, which can over-restrict a trailing general line — fails safe (over-restriction, never a leak). Marker chunks may exceed the token cap (kept whole for ACL clarity).
   - Future (if other document shapes appear): layout-aware heading detection (PyMuPDF font sizes or `unstructured`), per-source heading conventions in config, table-aware extraction.
